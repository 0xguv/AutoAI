...
<div class="h-full flex flex-col bg-dark-light">
  ...
  {:else if $activeTab === 'broll'}
    <div class="p-4 space-y-4">
      ...
      <!-- Auto Generate B-Roll Button -->
      <button
        on:click={generateBRollSuggestions}
        disabled={generatingBRoll || !$currentProject?.captions?.length}
        class="w-full p-3 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-lg font-medium text-white hover:from-yellow-600 hover:to-orange-600 transition disabled:opacity-50 flex items-center justify-center gap-2"
      >
        {#if generatingBRoll}
      ...
      <div class="border-t border-dark-lighter pt-4">
        <h4 class="font-medium text-sm text-dark-text-light mb-2">Search B-Roll</h4>
        <div class="flex gap-2">
          <input 
            type="text" 
